...
export async function sendWelcomeEmail(to: string, name: string) {
  const html = welcomeTemplate
    ...
    : `<div style="font-family: Arial, sans-serif;"><h1 style="color: #d4a017;">Welcome to IRONCORE Gym, ${name}!</h1><p>We're excited to have you join our community.</p></div>`;
  await sendEmail({ to, subject: 'Welcome to IRONCORE Gym!', html });
}
...
export async function sendBookingConfirmation(to: string, name: string, date: string, time: string) {
  ...
  await sendEmail({ to, subject: 'Booking Confirmed - IRONCORE Gym', html });
}
  ...
  await sendEmail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>Your trainer application at <strong>IRONCORE Gym</strong> has been approved.</p>
        ...
        <p style="color: #666;">- The IRONCORE Team</p>
      </div>
    `,
  });
}
  ...
  await sendEmail({
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p>Hi ${name},</p>
        <p>Thank you for your interest in joining <strong>IRONCORE Gym</strong> as a trainer.</p>
...
export async function sendPasswordResetEmail(to: string, name: string, resetLink: string) {
  await sendEmail({
    to,
    subject: 'Reset Your IRONCORE Gym Password',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
